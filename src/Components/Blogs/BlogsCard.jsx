  ...
  return (
    <div className="mx-auto max-w-[300px] md:max-w-[350px] space-y-6 rounded-xl bg-white px-4 pb-8 pt-4 font-sans shadow-lg  ">
      <Link
        ...
        className="relative flex h-48 w-full justify-center lg:h-[280px] "
      >
        <Swiper
          centeredSlides={true}
          autoplay={{
          ...
          <SwiperSlide>
            <div className="relative group">
              ...
              <Image
                width={300}
                height={300}
                className="h-[300px] w-full rounded-lg bg-black/40"
                src={
                  blog?.thumbnail ||
      ...
      <div className="mx-auto w-[90%] space-y-2  font-semibold">
        <div className="flex justify-between">
          <h6 className="text-sm md:text-base text-gray-800 lg:text-lg">
            {blog?.title}
          </h6>
        </div>
      </div>
      <div className="flex mx-auto flex-wrap w-[90%] items-center justify-between gap-6 text-sm md:text-base">
        <button onClick={handleReact} className={`flex items-center `}>
          <svg
            onClick={() => setBookmark(!bookMark)}
            width={30}
            className={`fill-transparent stroke-gray-500 stroke-2  hover:fill-red-500 hover:stroke-red-500 ${bookMark &&
        ...
        <Link
          ...
          className="border-2 icon-container rounded-full p-1 hover:bg-[#076aa5] border-[#076aa5] hover:text-white ease-in duration-300"
        >
          {hoverd ? (
            <IoMdArrowRoundForward
              size={22}
              className="icon-hover ease-in duration-300"
            ></IoMdArrowRoundForward>
          ) : (
            <MdArrowOutward
              size={22}
              className="icon-default ease-in duration-300"
            ></MdArrowOutward>
          )}
        </Link>
      </div>
