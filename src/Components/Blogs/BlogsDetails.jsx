...
const BlogsDetails = () => {
  ...
  const { isAuthenticated, logout, myInfo } = useAuth();
  ...
  const { data: blog, refetch } = useQuery({
    queryKey: ["blog", slug],
    queryFn: async () => {
      const res = await axiosPublic.get(`/blogs/${slug}`);
  ...
  const { data: comments, refetch: commentRefetch } = useQuery({
    queryKey: ["comment"],
    queryFn: async () => {
      const res = await axiosPublic.get(`/comment/${blog?._id}`);
  ...
  const handleReview = async (e) => {
    ...
    const commentObj = { blogId, content };

    const res = await axiosPublic.post(`/comment/create`, commentObj, {
  ...
  return (
    <div>
      <div className="container flex flex-col w-[95%] md:max-w-[70%] p-6 mx-auto  rounded-md ">
        <h2 className="text-xl md:text-2xl lg:text-4xl font-bold mb-5">
          {blog?.title}
        </h2>
        ...
        <div className="flex justify-between p-4 border-b-2">
          <div className="flex space-x-4">
            ...
            <div>
              <h4 className="font-bold">Md Imran</h4>
          ...
          <div className="flex items-center space-x-2 ">
            ...
            <span className="text-xl font-bold">{blog?.commentCount}</span>
          </div>
        </div>
        ...
        <h2 className="text-2xl font-bold my-11">
          Coments ({blog?.commentCount})
        </h2>
        ...
        <form
          ...
          className="flex text-white mt-8 flex-col w-full "
        >
          <textarea
            name="message"
            rows="3"
            placeholder="Message..."
            className="p-4 border-2 rounded-md resize-none text-black"
            spellCheck="false"
          ></textarea>
          ...
          <div className="flex w-full justify-between items-center ">
            <button
              type="submit"
              className="py-4 my-8 w-[180px] font-semibold rounded-md  bg-[#076aa5]"
            >
              Submit Comment
            </button>
        ...
        {show && (
          <div>
            {comments?.map((comment, index) => (
              <div key={index} className="mb-7 border-b-2">
                ...
                <p className="ml-[77px] text-gray-700 mb-6">
                  {comment?.content}
                </p>
